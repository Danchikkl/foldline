...
export function UploadDropzone() {
  ...
  async function handle(file?: File) {
    ...
    try {
      const presign = await fetch("/api/uploads/presign", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ filename: file.name, contentType: file.type, size: file.size }) });
      ...
      if (!put.ok) throw new Error("Upload to private storage failed.");
      setProgress(72);
      const complete = await fetch("/api/uploads/complete", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ documentId: data.documentId }) });
      const finished = await complete.json();
      if (!complete.ok) throw new Error(finished.error || "Upload verification failed.");
      setProgress(100);
      router.push(`/documents/${data.documentId}`);
      router.refresh();
    } catch (e) { setError(e instanceof Error ? e.message : "Upload failed."); setBusy(false); setProgress(0); }
  }
  ...
  return <section className={`dropzone ${drag ? "dropzoneActive" : ""}`} onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={(e) => { e.preventDefault(); setDrag(false); void handle(e.dataTransfer.files[0]); }}>
    ...
    <p>{busy ? "Upload → integrity check → OCR → Proof Mode" : "PDF or image, up to 25 MB. Your original stays private."}</p>
    ...
    <div className="dropMeta"><span><ShieldCheck size={14}/> Private R2 storage</span><span><Sparkles size={14}/> Review by exception</span></div>
  </section>;
}
